...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        # DEBUG - removing this step for debugging
        # x = self.norm1(x)
        x = x.view(B, H, W, C)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # moves downsampling out of this class
        # patch merging layer
...
class SwinTransformerFPN(BaseModule):
    r""" Swin Transformer
    ...
    model_names = {
        ...
        'swin_tiny_patch4_window7_224_moby': {
        ...
        #     'url': None,
        #     'arch': 'swin_tiny_c24_patch4_window8_256',
        # },
    ...
    configs = {
        'swin_tiny_patch4_window7_224': {
            ...
            'urls': {
                ...
                '_moby': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.3/moby_swin_t_300ep_pretrained.pth'},
            'num_stages': 4,
            'num_features': 768,
        },
        'swin_base_patch4_window12_384': {
            ...
            'urls': {
                ...
                '_22k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window12_384_22k.pth',
                '_22kto1k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window12_384_22kto1k.pth'},
            'num_stages': 4,
            'num_features': 1024,
        },
        'swin_large_patch4_window7_224': {
            ...
            'crop': False,
            'urls': {
                '_22k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_large_patch4_window7_224_22k.pth',
                '_22kto1k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_large_patch4_window7_224_22kto1k.pth'},
            'num_stages': 4,
            'num_features': 1536,
        },
        'swin_tiny_c24_patch4_window8_256': {
        ...
        'swin_base_patch4_window7_224': {
            ...
            'urls': {
                ...
                '_22k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window7_224_22k.pth',
                '_22kto1k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window7_224_22kto1k.pth'},
            'num_stages': 4,
            'num_features': 1024,
        },
        'swin_small_patch4_window7_224': {
        ...
        'swin_large_patch4_window12_384': {
            ...
            'crop': False,
            'urls': {
                '_22k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_large_patch4_window12_384_22k.pth',
                '_22kto1k': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_large_patch4_window12_384_22kto1k.pth'},
            'num_stages': 4,
            'num_features': 1536,
        },
    }
    ...
    def __init__(self,
        ...
        super(SwinTransformerFPN, self).__init__(None)
        
        arch = None
        if model_name is not None:
            assert model_name in self.model_names
            arch = self.model_names[model_name]['arch']
            self.config = self.configs[arch]
        ...
        # updated to move down-sampling step to the front of the following block
        self._dims = [
        ...
        for i_layer in range(self.num_stages):
            layer = BasicLayer(
                dim=self._dims[i_layer],
                input_resolution=self._ress[i_layer],
                ...
                norm_layer=norm_layer,
                # downsample=PatchMerging if (i_layer < self.num_stages - 1) else None,
                # downsample=PatchMerging if i_layer > 0 else None,
                use_checkpoint=use_checkpoint,
            )
            self.layers.append(layer)
            
            _pooling_layer = lambda v: v
            if i_layer > 0:
                _pooling_layer = PatchMerging(
                    input_resolution=self._ress[i_layer - 1],
                    dim=self._dims[i_layer - 1],
                    norm_layer=norm_layer,
                )
            self.pooling_layers.append(_pooling_layer)
        ...
        if model_name is not None:
            self.load_pretrain(name=model_name)
        self.to(device)
    ...
    def forward_features(self, x):
        ...
        for i_layer in range(self.num_stages):
            x = self.pooling_layers[i_layer](x)
            x = self.layers[i_layer](x)
            
            _out = x  # B L C
            B, L, C = _out.shape
            _out = nn.LayerNorm(C)(_out)
            _hw = int(self.window_size * (2 ** (self.num_stages - i_layer - 1)))
            assert L == _hw * _hw
            _out = torch.reshape(_out, [B, _hw, _hw, C])  # B H W C
            _out = _out.permute([0, 3, 1, 2]) # B C H W
            fpn_out.append(_out)
        
        return tuple(fpn_out)
        
        # x = self.norm(x)  # B L C
        
        # B, L, C = x.shape
        # x = torch.reshape(x, [B, self.window_size, self.window_size, C])  # B H W C
    ...
    #         flops += layer.flops()
    #     flops += self.num_features * self.patches_resolution[0] * self.patches_resolution[1] // (2 ** self.num_stages)
    #     flops += self.num_features * self.num_classes
    #     return flops
    
    def load_pretrain(self, name):
        assert isinstance(name, str)
        if name not in self.model_names or self.model_names[name]['url'] is None:
            raise ValueError('name [{}] not found! Use one of {}'.format(
                name,
                [k for k, v in self.model_names.items() if v['url'] is not None]
            ))
        
        _path = self.model_names[name]['url']
        if _path.startswith('https'):
            checkpoint = torch.hub.load_state_dict_from_url(
                _path, map_location='cpu', check_hash=True)
        else:
...
@BACKBONES.register_module()
class SwinTransformerFPNWrap(SwinTransformerFPN):
    
    def __init__(self,
                name='swin_tiny_patch4_window7_224',
                device='cuda',
                ):
        assert name in self.model_names
        arch = self.model_names[name]['arch']
        self.config = self.configs[arch]
        super(SwinTransformerFPNWrap, self).__init__(
            **self.config,
...
#     'swin_tiny_patch4_window7_224_moby': 'https://github.com/SwinTransformer/storage/releases/download/v1.0.3/moby_swin_t_300ep_pretrained.pth',
# }
...
#     }
#     _urls = {}
#     for k1, v1 in checkpoint_urls.items():
#         if not k1.startswith(k):
#             continue
#         subfix = k1[len(k):]
#         _config['urls'][subfix] = v1
#     _config['num_stages'] = len(_config['depths'])
#     _config['num_features'] = int(_config['embed_dim'] * 2 ** (_config['num_stages'] - 1))
#     configs_updated[k] = {**_config}

# %%
# json.dump(configs_updated, open('/home/hai/VT/ViT_detection_torch/configs_updated.json', 'w'), indent=4)
